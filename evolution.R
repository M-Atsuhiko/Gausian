evolution <- function(MULTI_GENERATION,Good_ID,Top_Estimate){
  ...
  MULTI_GENERATION[[Best_ID]][["TREE_volume"]] <- NULL
  MULTI_GENERATION[[Best_ID]][["Ca_Amount"]] <- NULL
  MULTI_GENERATION[[Best_ID]][["K_Amount"]] <- NULL
  ...
  MULTI_GENERATION[[Worst_ID]][["TREE_volume"]] <- NULL
  MULTI_GENERATION[[Worst_ID]][["Ca_Amount"]] <- NULL
  MULTI_GENERATION[[Worst_ID]][["K_Amount"]] <- NULL
...
#      cat("parent1: ",Good_ID[parent1_i],"(",parent1_i,") parent2: ",Good_ID[parent2_i],"(",parent2_i,")\n")
    ...
    MULTI_GENERATION[[IND_i]][["TREE_volume"]] <- NULL
    MULTI_GENERATION[[IND_i]][["Ca_Amount"]] <- NULL
    MULTI_GENERATION[[IND_i]][["K_Amount"]] <- NULL
  ...
  for(i_dend in 1:length(individual)){# 各パラメータ値をランダムにガウス乱数を用いて変更する
    ...
    for(param_i in 1:MAX_PARAMS){
      if(Mutation_or_Not[param_i] < GA_MUTATION_AR_G){
        ...
        else if(!(is.na(match(param_name,"K_Gaus_mean"))) ||
                !(is.na(match(param_name,"Ca_Gaus_mean")))){
          new_parameter <- max(min(rnorm(1,mean=parameter,sd=MUTATION_conductance_Gaus_mean),
                                   GAUS_MEAN_MAX),
                               GAUS_MEAN_MIN)
        }
        ...
        else if(!(is.na(match(param_name,"K_Gaus_sd"))) ||
                !(is.na(match(param_name,"Ca_Gaus_sd")))){
          new_parameter <- abs(rnorm(1,mean=parameter,sd=MUTATION_conductance_Gaus_sd))
        }
        ...
        else if(!(is.na(match(param_name,"K_peak")))){
          new_parameter <- abs(rnorm(1,mean=parameter,sd=MUTATION_K_peak))
          new_parameter <- min(new_parameter,K_MAX)*WITH_K
        }

        else if(!(is.na(match(param_name,"Ca_peak")))){
          new_parameter <- abs(rnorm(1,mean=parameter,sd=MUTATION_Ca_peak))
          new_parameter <- min(new_parameter,Ca_MAX)*WITH_Ca
        }
