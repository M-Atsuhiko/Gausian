divid_and_set_conductance <- function(TREE,Params){
  ...
  for(TREE_i in 1:length(TREE)){
    ...
    K_peak <- Params[[TREE_i]][["K_peak"]]
    K_Gaus_mean <- Params[[TREE_i]][["K_Gaus_mean"]]
    K_Gaus_sd <- Params[[TREE_i]][["K_Gaus_sd"]]
    Ca_peak <- Params[[TREE_i]][["Ca_peak"]]
    Ca_Gaus_mean <- Params[[TREE_i]][["Ca_Gaus_mean"]]
    Ca_Gaus_sd <- Params[[TREE_i]][["Ca_Gaus_sd"]]
    ...
    for(Branch in Dendrite){
      ...
      for(i in 1:nseg){
        ...
        path_ratio <- my_path/Dend_path
        
        K_conductance <- (dnorm(path_ratio,mean=K_Gaus_mean,sd=K_Gaus_sd)/K_Gaus_peak)*K_peak
        Ca_conductance <- (dnorm(path_ratio,mean=Ca_Gaus_mean,sd=Ca_Gaus_sd)/Ca_Gaus_peak)*Ca_peak
